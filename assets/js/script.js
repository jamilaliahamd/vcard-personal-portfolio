...
function buildImportQuotationFromRow(row){const customer=resolveImportField(row,['customer','customer_name','client','client_name','customer_name'])||'';const phone=resolveImportField(row,['phone','customer_phone','contact_phone','telephone','mobile'])||'';const project=resolveImportField(row,['project','project_name','job','job_name'])||'';const address=resolveImportField(row,['address','project_location','project_address','location'])||'';const quoteNumber=resolveImportField(row,['quotation_number','quote_number','quotationid','id','quote_id','quotation_id'])||'';const date=resolveImportField(row,['date','quotation_date','valid','validity','date_issued'])||new Date().toISOString().slice(0,10);
const status=resolveImportField(row,['status','state','quotation_status'])||'Draft';const material=resolveImportField(row,['material','item','product','name','description'])||'';const description=resolveImportField(row,['description','specification','spec'])||material||'';const quantity=parseImportNumber(resolveImportField(row,['quantity','qty','units','qty_required','qty_required']),0);
const unit=resolveImportField(row,['unit','uom'])||'each';const unitPrice=parseImportNumber(resolveImportField(row,['unit_price','selling_price','price','unitPrice','rate']),0);
const total=parseImportNumber(resolveImportField(row,['total','amount','line_total','total_amount']),0) || Math.max(0, quantity * unitPrice);
const amount=parseImportNumber(resolveImportField(row,['amount','grand_total','quote_amount','final_total']),0) || total;
if(!customer||!project||!material)return null;
...
function importQuotationExcel(file){if(!file)return;if(!window.XLSX){notify('Excel parsing library is not available');return;}const allowed=['xlsx','xls','csv'];const ext=String(file.name||'').split('.').pop()?.toLowerCase();if(!allowed.includes(ext)){notify('Please upload an Excel workbook (.xlsx,.xls) or CSV file.');return;}const reader=new FileReader();reader.onload=function(){try{const workbook=XLSX.read(reader.result,{type:'array',cellDates:true});const sheet=workbook.Sheets[workbook.SheetNames[0]];const rows=XLSX.utils.sheet_to_json(sheet,{defval:'',raw:false});const grouped=new Map();rows.forEach(row=>{const record=buildImportQuotationFromRow(row);if(!record)return;const key=resolveImportField(row,['quotation_number','quote_number','quotationid','id','quote_id','quotation_id'])||`${record.customer}||${record.project}`;const existing=grouped.get(key)||{customer:record.customer,customerPhone:record.customerPhone,project:record.project,projectLocation:record.projectLocation,valid:record.valid,date:record.date,status:record.status,status:record.status,items:[],amount:0};if(!existing.items)existing.items=[];existing.items.push(...record.items);existing.amount=Math.max(existing.amount, record.amount, existing.items.reduce((sum,item)=>sum+Number(item.total||0),0));if(!existing.id)existing.id=`QT-${String(Date.now()).slice(-5)}`;grouped.set(key,existing);});const imported=[];grouped.forEach(record=>{const normalized={id:`QT-${String(Date.now()).slice(-5)}-${Math.round(Math.random()*99)}`,customer:record.customer,customerPhone:record.customerPhone||'Phone not provided',project:record.project,projectLocation:record.projectLocation||'Location not provided',valid:record.valid||record.date||new Date().toISOString().slice(0,10),date:record.date||record.valid||new Date().toISOString().slice(0,10),status:record.status&&record.status.trim()?record.status.trim():'Draft',amount:Math.max(0,Number(record.amount)||record.items.reduce((sum,item)=>sum+Number(item.total||0),0)),items:record.items.map(item=>({item:item.item||'Material',description:item.description||'',quantity:Math.max(0,Number(item.quantity)||0),unit:item.unit||'each',unitPrice:Math.max(0,Number(item.unitPrice)||0),total:Math.max(0,Number(item.total)||0)}))};if(normalized.items.length){normalized.amount=Math.max(0,normalized.items.reduce((sum,item)=>sum+Number(item.total||0),0));db.quotations.unshift(normalized);imported.push(normalized);}});if(!imported.length){notify('No valid quotation records were found in the sheet.');return;}save();refresh();notify(`${imported.length} quotation${imported.length===1?'':'s'} uploaded from Excel`);}catch(error){notify('Unable to read the Excel sheet.');console.error(error);}};reader.readAsArrayBuffer(file);}
...
function notify(message='Saved successfully'){const toast=byId('toast');toast.querySelector('b').textContent=message;toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),2600);}
